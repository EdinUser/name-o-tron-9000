...
async fn poll_pin(
    ...
    let v: serde_json::Value =
        serde_json::from_str(&text).map_err(|e| format!("poll pin parse error: {e}"))?;
    let token = v
        .get("authToken")
        .and_then(|x| x.as_str())
        .or_else(|| v.get("auth_token").and_then(|x| x.as_str()))
        .map(|s| s.to_string());
    Ok((token, false))
}
...
pub async fn plex_login(app: tauri::AppHandle) -> Result<LoginStartResult, String> {
    ...
    {
        *LOGIN.lock().unwrap() = Some(LoginState {
            client_id: client_id.clone(),
            pin_id: pin.id,
            code: pin.code.clone(),
            started_at: Instant::now(),
            expires_in: if pin.expires_in > 0 {
                pin.expires_in
            } else {
                120
            },
    ...
    app.opener()
        .open_url(auth_url.clone(), Option::<String>::None)
    ...
    tokio::spawn(async move {
        ...
        while Instant::now() < deadline && status == LoginStatus::Pending {
            ...
            match poll_pin(&poll_client, &poll_client_id, poll_pin_id).await {
                Ok((maybe_token, expired)) => {
                    println!(
                        "Poll response {}: expired={}, has_token={}",
                        poll_count,
                        expired,
                        maybe_token.is_some()
                    );
                ...
                Err(e) => {
                    println!("Poll error {}: {}", poll_count, e);
        ...
        if let Some(login) = LOGIN.lock().unwrap().as_mut() {
            login.status = status;
            login.token = token.clone();
            println!(
                "Updated login state: status={:?}, has_token={}, poll_count={}",
                status,
                token.is_some(),
                poll_count
            );
...
pub fn plex_login_status() -> Result<LoginStatusResult, String> {
    ...
    if let Some(state) = &*guard {
        println!(
            "Current login status: {:?}, has_token: {}",
            state.status,
            state.token.is_some()
        );
...
mod tests {
    ...
    fn test_login_status_enum_serialization() {
        assert_eq!(
            serde_json::to_string(&LoginStatus::Pending).unwrap(),
            "\"pending\""
        );
        assert_eq!(
            serde_json::to_string(&LoginStatus::Authorized).unwrap(),
            "\"authorized\""
        );
        assert_eq!(
            serde_json::to_string(&LoginStatus::Error).unwrap(),
            "\"error\""
        );
        assert_eq!(
            serde_json::to_string(&LoginStatus::Expired).unwrap(),
            "\"expired\""
        );
        assert_eq!(
            serde_json::to_string(&LoginStatus::Idle).unwrap(),
            "\"idle\""
        );
    }
    ...
    fn test_login_status_deserialization() {
        assert_eq!(
            serde_json::from_str::<LoginStatus>("\"pending\"").unwrap(),
            LoginStatus::Pending
        );
        assert_eq!(
            serde_json::from_str::<LoginStatus>("\"authorized\"").unwrap(),
            LoginStatus::Authorized
        );
        assert_eq!(
            serde_json::from_str::<LoginStatus>("\"error\"").unwrap(),
            LoginStatus::Error
        );
        assert_eq!(
            serde_json::from_str::<LoginStatus>("\"expired\"").unwrap(),
            LoginStatus::Expired
        );
        assert_eq!(
            serde_json::from_str::<LoginStatus>("\"idle\"").unwrap(),
            LoginStatus::Idle
        );
    }
